...
public class TextDescriptor implements Cloneable{
    ...
    public StringBinding previewText = new StringBinding() {
        {
            bind(fontFamilyProperty, fontSizeProperty, fontColorProperty);
        }
        @Override
        protected String computeValue() {
            return fontFamilyProperty.get() + " (" + fontSizeProperty.get() + "): " + fontColorProperty.get();
        }
    };

    //specialni metoda ktera slouzi jako tovarna callbacku pro vytvoreni observablelistu objektu TextDescriptor, metoda preda pole pozorovatelnych property, ktere bude moci observable list kontrolovat a pozna jejich zmenu (bude tak reagovat na vnitrni zmeny svych objektu)
    public static Callback<TextDescriptor, Observable[]> extractor(){
        return new Callback<>() {
            public Observable[] call(TextDescriptor descriptor){
                return new Observable[]{descriptor.fontFamilyProperty(), descriptor.fontSizeProperty(), descriptor.fontColorProperty()};
            }
        };
    }
    ...
    //tovarna pro nahodny descriptor
    public static @NotNull
    TextDescriptor generateRandomDescriptor(){
        Random random = new Random();
        int size = random.nextInt(120) + 1;
        ...
        return new TextDescriptor(fonts.get(fontIndex), size, color);
    }

    //metoda vytvori hlubokou kopii textdescriptoru
    public TextDescriptor clone(){
        return new TextDescriptor(getFontFamily(), getFontSize(), new Color(getFontColor().getRed(), getFontColor().getGreen(), getFontColor().getBlue(), getFontColor().getOpacity()));// string(font) a int(size) jsou prim. dat. typy => mohu je rovnou predat, pozor na Color, pokud bych ji predal tak puvodni i klon budou sdilet jednu instanci
    }
